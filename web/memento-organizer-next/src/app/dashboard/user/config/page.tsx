...
import { MdAccessTimeFilled, MdAccountBox, MdKeyboardArrowDown, MdLockClock } from "react-icons/md"
    ...
    <>    
        <div className="sm:ml-0 ml-5 w-11/12 h-fit  bg-white dark:bg-slate-700 flex flex-col gap-4 drop-shadow-lg p-4 rounded-md mb-4">
           ...
           <div className="flex gap-2 items-end">
                ...
                <span className="text-sm sm:text-md md:text-lg">Last Login at {userData?.lastLogin}</span>
           </div>
           <div className="flex gap-2 items-end">
            <MdLockClock className="text-lg sm:text-xl md:text-3xl"/>
            <span className="text-sm sm:text-md md:text-lg ">Created at {userData?.issued}</span>

           </div>
        </div>
        <div className="w-full h-fit flex flex-col flex-shrink-0 flex-grow-0 gap-4 items-center">

        <AccountInfoAccordion fetchedEmail={userData?.email} refetchUserData={fetchData}/>
