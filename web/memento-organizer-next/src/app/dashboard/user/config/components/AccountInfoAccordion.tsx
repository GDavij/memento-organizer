...
export default function AccountInfoAccordion({fetchedEmail,refetchUserData}:AccountInfoAccordionProps) {
    
    const [open, setOpen] = useState(true);
    const [openEmailConfirmation, setOpenEmailConfirmation] = useState(false);
    const [openPassphraseConfirmation, setOpenPassphraseConfirmation] = useState(false);
    
        const emailForm = useForm<{email: string}>({defaultValues: {
            email: ""
        }});
        ...
        return (<>
         <Accordion open={open} closeOpen={() => setOpen((prev) => !prev)} subject="Account Data" height="h-[38rem] md:h-[20rem]">
            ...
            <form onSubmit={passphraseForm.handleSubmit((data) => {
                    if(passphraseForm.getValues("passphrase") != passphraseForm.getValues("confirmPassphrase"))
                    {
            ...
            className="flex flex-col md:flex-row items-center gap-8">
                <label htmlFor={passphraseId} className="w-full md:w-[35%] flex flex-col flex-shrink flex-grow-0">
                    ...
                    <div className="input-wrapper-left-flat">
                        <input id={passphraseId}{...passphraseForm.register("passphrase", {required: true, minLength: 16})} type="password" className="w-full h-8 bg-transparent p-6 outline-none" />
                ...
                <label htmlFor={confirmPassphraseId} className="w-full md:w-[35%] flex flex-col flex-shrink flex-grow-0">
                    ...
                    <div className="input-wrapper-left-flat">
                        <input id={confirmPassphraseId}{...passphraseForm.register("confirmPassphrase", {required: true, minLength: 16 })} type="password" className="w-full h-8 bg-transparent p-6 outline-none" />
                        <span>{passphraseForm.watch("confirmPassphrase")?.length || 0}</span>
                    </div>
                </label>
                <div className="w-full md:w-[30%]">
        ...
        <ConfirmDialog  onClose={() => setOpenEmailConfirmation(false)} open={openEmailConfirmation} onOkay={async () => {
       ...
       <ConfirmDialog  onClose={() => setOpenPassphraseConfirmation(false)} open={openPassphraseConfirmation} onOkay={async () => {
            ...
            try {
                const newToken = await userService.updateUser(localStorage.getItem('token')!,filteredBody);
