	...
	return (
		<>
				...
				{kanban?.columns &&
					kanban.columns.map((column) => (
						<div
							id={column.id}
							key={column.id}
							className='w-80 min-h-fit-content bg-slate-100 flex flex-col flex-grow-0 flex-shrink-0 overflow-y-auto'
							onDragOver={(ev) => {
								ev.preventDefault();
								...
								{kanbanTasks
									...
									.map((task) => (
										<div
											id={task.id}
											key={task.id}
											className='bg-white px-2 py-2   cursor-grab'
											draggable
											onDragStart={(ev) => {
										...
										>
											<div className='w-full flex justify-center items-center gap-2'>
												<div className='w-[0.2rem] h-4 bg-emerald-500 rounded-lgb'></div>
												<button
													className='w-fit h-fit bg-transparent border-0 outline-none cursor-pointer'
													onClick={(ev) => console.log('Clicked ' + task.id)}
												>
			...
			<CreateTaskModal
				...
				columnName={columnToCreateTask?.name || ''}
			/>
			<ToastContainer />
		</>
	);
