...
export default function Login() {
	...
	return (
		<>
			<main className='w-screen sm:h-screen sm:mt-0 flex flex-col gap-4 items-center sm:justify-center h-fit mt-10'>
				<form
					className='flex flex-col p-8 bg-white dark:bg-slate-700 sm:w-6/12 h-fit gap-6 sm:rounded-lg rounded-none'
					onSubmit={handleSubmit(handleLogin)}
					method='POST'
				>
					<h1 className='text-2xl'>Login users</h1>
				...
				<div className='fixed bottom-0 left-0 sm:relative w-full p-4 sm:w-6/12 h-fit gap-6 sm:rounded-lg rounded-none rounded-t-lg bg-white dark:bg-slate-700'>
					<Link href='/login/admin' className='button-flat-inverse p-4'>
						I am Admin
					</Link>
				</div>
			</main>
